...
def run_benchmark(custom, args):
    ...
    configs = []
    plot_name = f'fused-attention-{mode}-d{head_size}-layout{args.layout}-causal{args.causal}-type{args.dtype}'
    ...
    line_vals = ['triton', 'perf_report']
    line_names = [args.branch, 'perf_report']  # 'Time (ms)' if print_time else 'TFLOPS'
    configs.append(
        triton.testing.Benchmark(x_names=x_names, x_vals=x_vals_list, line_arg='provider', line_vals=line_vals,
                                 line_names=line_names, styles=[('green', '-'), ('red', '-')],
    ...
    def bench_flash_attention(BATCH, HQ, HK, N_CTX_Q, N_CTX_K, D_HEAD, dtype, causal, mode, provider, device="cuda",
        ...
        elif "perf_report" in provider:
            # Pre-defined perf_report results (D=128, H=64)
            # Key: (dtype_str, causal, batch, n_ctx) -> tflops
            perf_report_results = {
                # fp16 causal=0
                ('fp16', False, 32, 512): 669,
                ('fp16', False, 16, 1024): 873,
                ('fp16', False, 8, 2048): 1024,
                ('fp16', False, 4, 4096): 1073,
                ('fp16', False, 2, 8192): 1099,
                ('fp16', False, 1, 16384): 1112,
                # fp16 causal=1
                ('fp16', True, 32, 512): 343,
                ('fp16', True, 16, 1024): 548,
                ('fp16', True, 8, 2048): 739,
                ('fp16', True, 4, 4096): 894,
                ('fp16', True, 2, 8192): 985,
                ('fp16', True, 1, 16384): 949,
                # bf16 causal=0
                ('bf16', False, 32, 512): 689,
                ('bf16', False, 16, 1024): 898,
                ('bf16', False, 8, 2048): 1080,
                ('bf16', False, 4, 4096): 1143,
                ('bf16', False, 2, 8192): 1168,
                ('bf16', False, 1, 16384): 1183,
                # bf16 causal=1
                ('bf16', True, 32, 512): 344,
                ('bf16', True, 16, 1024): 559,
                ('bf16', True, 8, 2048): 770,
                ('bf16', True, 4, 4096): 924,
                ('bf16', True, 2, 8192): 1065,
                ('bf16', True, 1, 16384): 1011,
            }
            # Map torch dtype to string
            dtype_str = 'fp16' if dtype == torch.float16 else 'bf16'
            key = (dtype_str, causal, BATCH, N_CTX_Q)
            if key in perf_report_results:
                return perf_report_results[key]
            else:
                assert False, f"No perf_report result for key {key}"
        else:
...
def parse_args():
    ...
    parser.add_argument("-branch", default='upstream')
